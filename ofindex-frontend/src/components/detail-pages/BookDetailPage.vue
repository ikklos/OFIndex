...
const reloadAvailablePackList = () => {
  ...
  axiosApp().then(app => {
    app.get('/user').then(res => {
      app.get('/pack/user/' + res.data.userId + '/' + bookData.bookId).then(response => {
...
const flagLike = function (index) {
  ...
  if (packData.value[index].liked === false) {
    ...
    axiosApp().then(app => {
      app.delete('/pack/like/' + id).then(res => {
...
onMounted(() => {
  if (bookData.bookId !== null) {
    //发送详情请求，返回数据
    axiosApp().then(app=>{
      app.get('/book/' + bookData.bookId).then((response) => {
      ...
      app.get('/search/pack/' + bookData.bookId).then((res) => {
      ...
      <el-row>
        <el-col :span="8">
          <div class="book-left-area">
            ...
            <el-row class="book-cover-area">
              <el-image :src="bookData.cover" class="book-cover-img" fit="cover"></el-image>
            </el-row>
            <el-row class="book-isbn-area">ISBN:{{ bookData.isbn }}</el-row>
            <el-row class="book-tags-area">tags:
              <el-tag v-for="(item,index) in bookData.tag">{{ item }}</el-tag>
            </el-row>
          </div>
        </el-col>
                  ...
                  <div v-for="(item,index) in packData" :key="index" class="pack-item">
                    <el-row class="pack-item-inner">
                      <el-col :span="5" class="pack-avatar">
                        <el-avatar :src="item.authorAvatar" fit="cover"
                                   style="height: 60px; width: 60px; border: 1px solid #3c5cd7"></el-avatar>
                      </el-col>
                      <el-col :span="19">
                        <el-row>
                          <el-col :span="8"
                                  style="background: #fcfce5; border-radius: 5px; font-size:18px; line-height: 20px; padding:5px; box-sizing: border-box">
                            {{ item.name }}
                          </el-col>
                          <el-col :span="16">
                            {{ item.likeCount }}
                            <Icon type="ios-heart-outline" class="like-icon" @click="flagLike(index)"
                                  v-if="!item.liked"/>
                            <Icon type="ios-heart" class="like-icon" @click="flagLike(index)" v-if="item.liked"/>
                            <el-button color="#4825f6" style="border-radius: 20px" @click="copyPack(index)">
                              添加此资源包
                            </el-button>
                          </el-col>
                        </el-row>
